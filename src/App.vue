  ...
  <div id="app">
    <div class="main">
      <div class="calendar-holder">
        <FullCalendar :config="config" :events="events" @event-selected="eventClick" @event-drop="eventDrop"
          @event-resize="eventResize" @event-render="eventRendered" />
      </div>

      <div v-if="showModal" @close="showModal = false">
        <EventForm :event="currentEvent" :method="currentMethod" :list="projects" :events="events"
          @addProject="addProject" @close="closeModal" @removeProject="removeProject" />
      </div>
      ...
      <b-container class="form-holder" id="external-events">
        ...
        <div id='external-events-listing'
          style="overflow: scroll; margin-top: 20px; max-height: 600px; margin-bottom: 30px; overflow-x:hidden;">
          <b-row v-for="item in projects" :key="item.id">
            <div style="display: inline-flex; margin-left: 40px;" :class="{selected: selected === item.id}">
              <div class="colorbox" :style="{ background: item.backgroundColor}"
                @click="currentEvent = item ,currentMethod = 1, showModal = true" />
              <div style="text-align: left; width: 180px; padding: 1px; cursor: default;"
                @click="currentEvent = item ,currentMethod = 1, showModal = true">
                {{ item.title }} <br />
                ({{ item.id}})
              </div>
        ...
        <div>
          <b-row>
            <b-col><button class="btn btn-primary" @click="currentMethod = 0, showModal = true">+ add project</button>
            </b-col>
            <b-col><button class="btn btn-primary" @click="currentMethod = 3, showModal = true">+ add event</button>
            </b-col>
          </b-row>
        </div>
        ...
        while (startDate <= endDate) {
          ...
          for (i = 0; i < this.events.length; i++) {
            if (new Date(this.events[i].start) >= startDate && new Date(this.events[i].start) <= nextDay) {
              ...
              text = '\t\t' + this.events[i].start.split(" ")[1] + " - " + this.events[i].end.split(" ")[1] + ' - ' +
                proId + ' - ';
